  ...
  fields: (t) => ({
    ...
    books: t.connection({
      ...
      resolve: async (parent, args, context) => {
        const allBooks = await context.loaders.booksByAuthor.load(parseInt(parent.id as string));
        ...
        let startIndex = 0;
        if (args.after) {
          const cursorIndex = sortedBooks.findIndex(book => book.id === parseInt(args.after ?? ''));
          startIndex = cursorIndex + 1;
        }
        
        const paginatedBooks = sortedBooks.slice(startIndex, startIndex + take + 1);
        
        const hasNextPage = paginatedBooks.length > take;
        const nodes = hasNextPage ? paginatedBooks.slice(0, -1) : paginatedBooks;
